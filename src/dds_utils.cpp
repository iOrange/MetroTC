...
void DDS_DecodeColorBlock(uint8_t* dest, const size_t w, const size_t h, const size_t xOff, const size_t yOff, const bool isBC3, const uint8_t* src) {
    ...
    const uint16_t c0 = reinterpret_cast<const uint16_t*>(src)[0];
    const uint16_t c1 = reinterpret_cast<const uint16_t*>(src)[1];
...
void DDS_DecodeBC2AlphaBlock(uint8_t* dest, const size_t w, const size_t h, const size_t xOff, const size_t yOff, const uint8_t* src) {
    for (size_t y = 0; y < h; ++y) {
        uint8_t* dst = dest + yOff * y;
        uint32_t alpha = reinterpret_cast<const uint32_t*>(src)[y];
        for (size_t x = 0; x < w; ++x) {
            *dst = (alpha & 0xF) * 17;
            alpha >>= 4;
            dst += xOff;
...
void DDS_DecodeBC3AlphaBlock(uint8_t* dest, const size_t w, const size_t h, const size_t xOff, const size_t yOff, const uint8_t* src) {
    ...
    uint64_t alpha = *reinterpret_cast<const uint64_t*>(src) >> 16;

    for (size_t y = 0; y < h; ++y) {
        uint8_t* dst = dest + yOff * y;
        for (size_t x = 0; x < w; x++) {
            const uint32_t k = static_cast<uint32_t>(alpha & 0x7);
            if (0 == k) {
                *dst = a0;
            } else if (1 == k) {
                *dst = a1;
...
void DDS_DecompressBC1(const void* inputBlocks, void* outPixels, const size_t width, const size_t height) {
    ...
    const uint8_t* src = reinterpret_cast<const uint8_t*>(inputBlocks);
    uint8_t* dest = reinterpret_cast<uint8_t*>(outPixels);
...
void DDS_CompressBC3(const void* inputRGBA, void* outBlocks, const size_t width, const size_t height) {
    ...
    const uint8_t* srcPtr = reinterpret_cast<const uint8_t*>(inputRGBA);
    uint8_t* dst = reinterpret_cast<uint8_t*>(outBlocks);
...
void DDS_MakeDX10Headers(DDSURFACEDESC2& desc, DDS_HEADER_DXT10& dx10hdr, const size_t w, const size_t h, const size_t numMips, const bool isCube) {
    ...
    desc.dwWidth = static_cast<uint32_t>(w);
    desc.dwHeight = static_cast<uint32_t>(h);
    ...
    if (numMips > 1) {
        ...
        desc.dwMipMapCount = static_cast<uint32_t>(numMips);
    }
...
void DDS_MakeDX9Header(DDSURFACEDESC2& desc, const size_t w, const size_t h, const size_t numMips, const bool isBC1) {
    memset(&desc, 0, sizeof(desc));
    ...
    desc.ddpfPixelFormat.dwSize = sizeof(DDPIXELFORMAT);
    desc.ddpfPixelFormat.dwFlags = DDPF_FOURCC;
    desc.ddpfPixelFormat.dwFourCC = isBC1 ? PIXEL_FMT_DXT1 : PIXEL_FMT_DXT5;
    desc.ddsCaps.dwCaps = DDSCAPS_TEXTURE;
